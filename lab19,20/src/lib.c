...
void read_from_file(list* head)
{
  ...
  for(size_t i = 0; i < count; i++)
  {
    ...
    if (strcmp(audience, "VK302") == 0)
    {
      current->audience = 0;
    }
    else if (strcmp(audience, "EK204") == 0)
    {
      current->audience = 1;
    }
    else if (strcmp(audience, "EK302") == 0)
    {
      current->audience = 2;
    }
    else if (strcmp(audience, "Karazina705") == 0)
    {
      current->audience = 3;
    }
  ...
  fwrite = fopen("write.txt", "w");
    ...
    while(temp != NULL)
    {
      if (temp->every_week == true)
  		...
  		if (temp->audience == 0)
  		{
        fprintf(fwrite, "%s ", "ВK302");
  		}
  		else if (temp->audience == 1)
  		{
        fprintf(fwrite, "%s ", "EK204");
  		}
  		else if (temp->audience == 2)
  		{
        fprintf(fwrite, "%s ", "EK302");
  		}
  		else if (temp->audience == 3)
  		{
        fprintf(fwrite, "%s ", "Каразина705");
  		}
